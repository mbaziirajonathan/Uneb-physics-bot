...
def run(level):
    ...
    topics = {
        "S1": ["Select Topic", "Introduction to Chemistry", "Lab Apparatus", "Bunsen Burner", "States of Matter", "Mixtures", "Compounds", "Elements"],
        "S2": ["Select Topic", "Atomic Structure", "The Periodic Table", "Titration", "Acids, Bases and Salts", "Air and Combustion", "Water"],
        "S3": ["Select Topic", "Chemical Bonding", "Water Molecule", "Rates of Reaction", "Carbonates", "Extraction of Metals", "Organic Chemistry Intro"],
        "S4": ["Select Topic", "Organic Chemistry", "Alkane", "Alkenes", "Alcohols", "Electrochemistry", "Nitrogen and Fertilizers"]
    }
    ...
    if st.button("Generate Lesson", type="primary", key=f"btn_chem_{level}"):
        ...
        with st.spinner("AI Teaching..."):
            res = client.chat.completions.create(model="llama-3.1-8b-instant", messages=[{"role": "system", "content": f"You are UNEB {level} Chemistry tutor. Use NCDC syllabus."}, {"role": "user", "content": f"Teach {topic} with definition, example, 1 practice question"}])
