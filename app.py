    ...
    with tab2:
        ...
        if mode == "📖 Theory":
            ...
            if st.button("Generate AOI Project"):
                raw = ask_smart_brain(f"Design AOI project for {level2} {subject2} on {topic2}", subject2, level2, topic2); display_with_pdf(raw, f"AOI_{topic2}")
            ...
            if st.button("Generate Full Practical"):
                report = generate_practical(subject2,level2,prac); display_with_pdf(report, f"Practical_{prac}")
        elif mode == "📝 Quiz Mode":
            ...
            if st.button("Generate 20 ITEMS", type="primary"):
                bulk = generate_bulk_revision(subject2, level2); display_with_pdf(bulk, f"BulkRevision_{subject2}_{level2}")

# ========== 7. ADMIN PORTAL - FIXED INDENTATION ==========
def show_admin_portal():
    ...
    if selected == "Admin Dashboard":
        ...
        if st.button("Generate"):
            display_with_pdf(generate_exam_items(f"Generate {n} on {t}", s, l), "Test")
        ...
        if st.button("Generate"):
            display_with_pdf(generate_lesson_plan(s,l,t,d), "LessonPlan")

    elif selected == "Single Report Card":
        name = st.text_input("Name")
        scores = {sub: st.number_input(sub,0,100) for sub in ["Math","English","Science"]}
        if st.button("Generate"):
            display_with_pdf(generate_report_card(f"Name: {name}\n{scores}"), "Report")

    elif selected == "BULK EXAMS GENERATOR":
        s = st.selectbox("Subject", list(UNEB_CURRICULUM_MAP.keys()))
        l = st.selectbox("Class", [f"S{i}" for i in range(1,7)])
        if st.button("Generate 20"):
            display_with_pdf(generate_bulk_revision(s,l), "Bulk")
        ...
        if st.button("Add"):
            st.session_state.students_db.append({"name": name}); st.success("Added")
        ...
        if st.button("Save"):
            st.session_state.qbank.append({"q": q}); st.success("Saved")
        ...
        if st.button("Generate SOW"):
            ...
            sow = "\n".join([f"Week {i+1}: {t}" for i,t in enumerate(topics)])
            display_with_pdf(sow, "SOW")
    else:
        st.info(f"{selected} UI Ready")
