...
st.sidebar.caption("Build: V5.2.7-FIX-KEYS") # <-- This proves new code is running
...
st.sidebar.success(f"⚠️ DIGITAL UNEB TUTOR 2026 PRO V5.2.7\nSINGLE KEY MODE\n📞 {CONTACT}")
...
def display_with_preview(content, name):
    edited = st.text_area("AI Preview - EDIT BEFORE DOWNLOAD", content, height=350, key=f"preview_{name}_{time.time()}")
    cols = st.columns(4)
...
### 7. STUDENT PORTAL - 100% UNIQUE KEYS ###
def show_student_portal():
    ...
    with tab4:
        ...
        topic4 = st.selectbox("Topic", UNEB_CURRICULUM_MAP[subject4][level4], key="s4_topic") # <-- KEY ADDED
...
st.title("🎓 DIGITAL UNEB TUTOR 2026 PRO - V5.2.7")
