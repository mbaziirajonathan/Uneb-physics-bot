...
def universal_search(client, query, subject, level, topic, history):
    system = f"""ROLE: You are a UNEB {subject} tutor assistant for {level} Uganda NCDC 2026.
    RULES: 1. YOU ONLY TEACH {subject}. 2. IF QUESTION IS NOT ABOUT {subject}, SAY: "I only teach {subject}. Please select {subject} in the sidebar." 3. NEVER MENTION OTHER SUBJECTS. 4. GIVE MULTIPLE EXAMPLES WHEN ASKED. 5. FOLLOW NCDC 2026 SYLLABUS ONLY."""
    user = f"SUBJECT: {subject} LEVEL: {level} CURRENT TOPIC: {topic} STUDENT QUERY: '{query}'. Provide: 1.Direct Answer 2.MINIMUM 3 WORKED EXAMPLES 3.UNEB Style Questions 4.Common mistakes 5.Exam tip. Use only {subject} content."
...
def explain_practical_detailed(client, subject, level, topic, history):
    system = f"ROLE: You are a UNEB {subject} examiner 2026. RULE: Only explain {subject} practicals following NCDC 2026 guidelines. GIVE MULTIPLE EXAMPLES."
    user = f"SUBJECT: {subject} LEVEL: {level} PRACTICAL: {topic}. Provide: 1.DETAILED EXPLANATION 2.REQUIREMENTS/APPARATUS 3.RULES STUDENT MUST FOLLOW 4.CRITICAL POINTS TO SCORE FULL MARKS 5.GRAPH DESCRIPTION with labeled axes 6.MINIMUM 2 ILLUSTRATED EXAMPLES of similar practicals. Be specific for UNEB P3 2026."
    msgs = [{"role": "system", "content": system}] + history[-2:] + [{"role": "user", "content": user}]
    return call_groq(client, msgs, subject)

def explain_mistake(client, subject, level, question, wrong_answer, topic, history):
    system = f"ROLE: You are a UNEB {subject} examiner for {level} Uganda. RULE: Only mark {subject} questions. GIVE MULTIPLE CORRECT EXAMPLES."
    user = f"SUBJECT: {subject} LEVEL: {level} TOPIC: {topic} Q: {question}\nStudent Wrong Answer: {wrong_answer}\nExplain: 1.Why wrong 2.What UNEB expects 3.HOW TO GET FULL MARKS 4.GIVE 3 CORRECT WORKED EXAMPLES 5.Related {subject} concept."
    msgs = [{"role": "system", "content": system}] + history[-2:] + [{"role": "user", "content": user}]
    return call_groq(client, msgs, subject)

def generate_quiz(client, subject, level, topic, history):
    system = f"ROLE: You are a UNEB {subject} tutor assistant 2026. RULE: Only generate {subject} questions. PROVIDE MULTIPLE QUESTION TYPES."
    user = f"SUBJECT: {subject} LEVEL: {level} TOPIC: {topic}. Generate 10 MCQ + 3 Short Answer + 1 Calculation. Format clearly. Only {subject} syllabus NCDC 2026."
    msgs = [{"role": "system", "content": system}] + history[-2:] + [{"role": "user", "content": user}]
    return call_groq(client, msgs, subject)

def generate_marking_scheme(client, subject, level, question, student_answer, topic, history):
    system = f"ROLE: You are a UNEB {subject} Head Examiner 2026 for {level}. RULE: Mark only {subject}. GIVE MULTIPLE MODEL ANSWERS."
    user = f"SUBJECT: {subject} LEVEL: {level} TOPIC: {topic} Q: {question}\nStudent: {student_answer}\nGenerate: 1.MODEL ANSWER 2.ALTERNATIVE 2 MODEL ANSWERS 3.MARKING GUIDE 4.COMMON MISTAKES 5.TIPS TO SCORE 10/10."
    msgs = [{"role": "system", "content": system}] + history[-2:] + [{"role": "user", "content": user}]
    return call_groq(client, msgs, subject)
...
def generate_apparatus_list(client, practical_topic, subject, history):
    ...
    user = f"SUBJECT: {subject} PRACTICAL: {practical_topic}. List apparatus for 40 students. Include cost in UGX, local alternatives, and safety rules."
...
def generate_practical(client, subject, level, topic, history):
    system = f"ROLE: You are a UNEB {subject} examiner for {level} Uganda NCDC 2026. RULE: Only {subject} practicals. INCLUDE EXAMPLES."
    user = f"SUBJECT: {subject} TOPIC: {topic}. Generate complete practical report. Include AIM,HYPOTHESIS,VARIABLES,APPARATUS,PROCEDURE,SAFETY,DATA TABLE,GRAPH GUIDE WITH LABELING,2 EXAMPLES,CONCLUSION. MUST END with JSON data: ```json {{\"x_label\": \"X\", \"y_label\": \"Y\", \"data\": [[1,2],[2,4],[3,6],[4,8],[5,10],[6,12]]}} ```"
    msgs = [{"role": "system", "content": system}] + [{"role": "user", "content": user}]
    return call_groq(client, msgs, subject)

def describe_and_draw_graph(client, prompt, subject, level, topic, history):
    system = f"ROLE: You are a UNEB {subject} examiner Uganda 2026. RULE: Only {subject} graphs. GIVE EXAMPLES."
    user = f"SUBJECT: {subject} LEVEL: {level} TOPIC: {topic} GRAPH: {prompt}. Return JSON data then 3 EXAMPLES of similar graphs and 3 UNEB marking points."
    msgs = [{"role": "system", "content": system}] + [{"role": "user", "content": user}]
    return call_groq(client, msgs, subject)

def describe_uploaded_graph(client, image_bytes, subject):
    b64 = base64.b64encode(image_bytes).decode(); system = f"ROLE: You are a UNEB {subject} examiner. RULE: Analyze only {subject} graphs. GIVE EXAMPLES."; user = f"SUBJECT: {subject} Describe this graph image, what {subject} experiment it represents, and give 2 similar UNEB examples for 2026."
...
def voice_chat(client, audio_bytes, subject, level, topic, history):
    ...
    user_text = transcription.text; system = f"ROLE: You are a UNEB {subject} tutor assistant for {level} Uganda NCDC 2026. RULE: Only answer {subject}. GIVE EXAMPLES."
    msgs = [{"role": "system", "content": system}] + history[-2:] + [{"role": "user", "content": f"SUBJECT: {subject} LEVEL: {level} TOPIC: {topic} Question: {user_text}. Answer with 1 direct answer and 2 examples."}]
    res = client.chat.completions.create(model="llama-3.1-8b-instant", messages=msgs, temperature=0.3, max_tokens=400)
...
def generate_prediction(client, subject, paper, level, history):
    system = f"ROLE: You are a UNEB {subject} Head of Examinations 2026. RULE: Only {subject} papers. GIVE MULTIPLE VARIANTS."
    prompts = {"P1": f"SUBJECT: {subject} LEVEL: {level} Generate 40 MCQ for {subject} Paper 1. Mix {subject} topics S1-S4. 4 options A-D. NCDC 2026. Include 3 variants per difficult question.","P2": f"SUBJECT: {subject} LEVEL: {level} Generate 5 Theory questions for {subject} Paper 2. S3-S4. Include 2 calculations with 2 worked examples each, 1 diagram. 10 marks each. Marking guide.","P3": f"SUBJECT: {subject} LEVEL: {level} Generate 3 Practical scenarios for {subject} Paper 3. Competency-based NCDC 2026. Include apparatus and 2 examples per practical."}
...
def ask_box(client, key, subject, level, topic):
    st.text_input("❓ Ask anything about this topic:", key=f"ask_{key}")
    if st.button("Ask Tutor", key=f"btn_{key}"):
        query = st.session_state[f"ask_{key}"]
        if query:
            with st.spinner("Thinking..."):
                answer = universal_search(client, query, subject, level, topic, st.session_state.chat_history)
...
def main():
    ...
    <div style="background-color:#0E4D92; padding:12px; border-radius:10px; margin-bottom:15px">
        <h3 style="color:white; margin:0; text-align:center">
            📌 Current: {subject} | Class: {level} | Topic: {topic} | Mode: {mode}
        </h3>
    </div>
    ...
    if mode == "🔍 Smart Search":
        ...
        search_q = st.text_input("Search:", placeholder="e.g. Give 3 examples of Ohm's Law, Acid and Base reactions, Photosynthesis factors")
        if st.button("Search", use_container_width=True):
            if search_q:
                with st.spinner("Searching UNEB database..."):
                    result = universal_search(client, search_q, subject, level, topic, st.session_state.chat_history)
        ...
        st.title(f"Theory: {subject} {level} - {topic}"); col1,col2 = st.columns([1.2,1])
        with col1:
            if st.button("Generate UNEB Notes + 3 Examples", use_container_width=True):
                with st.spinner("Generating notes..."):
                    prompt = f"ROLE: UNEB {subject} tutor. SUBJECT: {subject} LEVEL: {level} TOPIC: {topic}. Give 6 concise UNEB exam notes + 3 WORKED EXAMPLES. NCDC Uganda syllabus 2026. Competency based."
                    res = client.chat.completions.create(model="llama-3.1-8b-instant", messages=[{"role":"system","content":f"You are UNEB {subject} tutor only."},{"role":"user","content":prompt}], max_tokens=600); st.session_state.notes = res.choices[0].message.content; st.session_state.activities_log.append(f"Generated {subject} notes for {topic}")
            st.divider()
            ...
            st.divider(); ask_box(client, "theory", subject, level, topic)
        ...
        if st.button(f"📖 Explain Full Practical + Examples: {practical_topic}", use_container_width=True):
            with st.spinner("Generating detailed explanation..."):
                explanation = explain_practical_detailed(client, subject, level, practical_topic, st.session_state.chat_history)
                st.markdown(explanation); st.session_state.activities_log.append(f"Explained {subject} Practical: {practical_topic}")
        st.divider()
        if st.button(f"💰 Get Apparatus & Budget: {practical_topic}", use_container_width=True):
            with st.spinner("Calculating budget..."): budget = generate_apparatus_list(client, practical_topic, subject, st.session_state.chat_history); st.markdown(budget); st.session_state.activities_log.append(f"Budget for {subject}: {practical_topic}")
        st.divider()
        if st.button(f"Generate Full Report + Graph + Examples: {practical_topic}", use_container_width=True):
            with st.spinner("System generating full UNEB report..."):
                report = generate_practical(client,subject,level,practical_topic, st.session_state.chat_history)
                data, json_block = safe_json_extract(report)
                if not data or "data" not in data:
                    st.warning("AI failed to generate data. Using default data so graph works.")
                    data = generate_default_data(practical_topic)
                try:
                    df = pd.DataFrame(data["data"], columns=[data["x_label"], data["y_label"]])
                    st.dataframe(df, use_container_width=True)
                    render_graph(df,data["x_label"],data["y_label"],practical_topic)
                ...
                st.session_state.activities_log.append(f"{subject} Practical report: {practical_topic}")
        st.divider(); ask_box(client, "practical", subject, level, topic)
        ...
        with tab1:
            ...
            if st.button("Generate & Draw Graph + Examples", use_container_width=True, key="btn_desc"):
                if not user_graph.strip(): st.warning("Please describe a graph first.")
                else:
                    with st.spinner("Generating graph..."):
                        result = describe_and_draw_graph(client, user_graph, subject, level, topic, st.session_state.chat_history); data, json_block = safe_json_extract(result)
            ...
            st.divider(); ask_box(client, "graph", subject, level, topic)
        with tab2:
            ...
            if st.button("Analyze Graph + Examples", use_container_width=True, key="btn_analyze"):
        ...
        st.title("🎙️ Voice Chat Tutor"); st.write(f"Talk to the tutor about {subject} {level} - {topic}.")
        ...
        with st.spinner("Listening and processing..."):
            user_q, tutor_a, tutor_audio = voice_chat(client, audio['bytes'], subject, level, topic, st.session_state.chat_history)
        ...
        st.title(f"📝 UNEB Quiz Mode: {subject} {level} - {topic}")
        if st.button("Generate 10 MCQ + 3 Short + 1 Calc + Examples", use_container_width=True):
        ...
        st.divider(); ask_box(client, "quiz", subject, level, topic)

    elif mode == "🧠 Mistake Explainer":
        st.title(f"🧠 Mistake Explainer: {subject} {level} - {topic}"); st.write("Paste student wrong answer. I will explain with 3 correct examples")
        qn = st.text_area("UNEB Question:"); wrong = st.text_area("Student Wrong Answer:")
        if st.button("Explain Mistake + 3 Examples", use_container_width=True):
            if qn and wrong:
                with st.spinner("Analyzing..."): explanation = explain_mistake(client, subject, level, qn, wrong, topic, st.session_state.chat_history); st.error(explanation); st.session_state.activities_log.append(f"Explained mistake in {subject}")
            else: st.warning("Enter both question and answer")

    elif mode == "💬 Chat Memory":
        st.title("💬 Chat Memory - Yesterday's Explanations")
        mem_q = st.text_input("Search Memory:", placeholder="e.g. give examples of titration we discussed")
        if st.button("Recall + Examples", use_container_width=True):
            if mem_q:
                context = "\n".join([f"{m['role']}: {m['content']}" for m in st.session_state.chat_history[-10:]])
                with st.spinner("Searching memory..."):
                    result = universal_search(client, f"Based on this chat history: {context}. Now answer with examples: {mem_q}", subject, level, topic, [])
                    st.markdown(result)

    elif mode == "🔮 Predict Papers":
        st.title(f"🔮 UNEB 2026 Prediction: {subject} {level}"); st.info("Predictions based on UNEB trends 2016-2023 + NCDC 2026.")
        c1,c2,c3 = st.columns(3)
        with c1:
            if st.button("Generate P1 MCQ + Variants", use_container_width=True):
                with st.spinner("..."): st.session_state.p1 = generate_prediction(client,subject,"P1", level, st.session_state.chat_history); st.session_state.activities_log.append(f"Generated {subject} P1")
            if "p1" in st.session_state: st.text_area("Paper 1", st.session_state.p1, height=400)
        with c2:
            if st.button("Generate P2 Theory + Examples", use_container_width=True):
