...
def s1_bio_animal_cell():
    ...
    return """<svg width="320" height="320"><circle cx="160" cy="160" r="110" fill="none" stroke="black" stroke-width="2"/><circle cx="160" cy="160" r="35" fill="#E3F2FD" stroke="black"/><ellipse cx="100" cy="100" rx="12" ry="6" fill="#FFCC80" stroke="black"/><circle cx="200" cy="110" r="15" fill="#F8BBD0" stroke="black" stroke-dasharray="2,2"/><line x1="195" y1="160" x2="250" y2="160" stroke="black"/><text x="255" y="164" font-size="11">Cell Membrane</text><line x1="160" y1="125" x2="160" y2="70" stroke="black"/><text x="165" y="70" font-size="11">Nucleus</text><line x1="100" y1="100" x2="50" y2="80" stroke="black"/><text x="10" y="80" font-size="11">Mitochondrion</text><line x1="200" y1="110" x2="250" y2="90" stroke="black"/><text x="255" y="90" font-size="11">Vacuole</text><line x1="130" y1="190" x2="90" y2="220" stroke="black"/><text x="30" y="225" font-size="11">Cytoplasm</text><text x="160" y="300" font-size="14" text-anchor="middle">Animal Cell</text></svg>"""

def s1_bio_plant_cell():
    """S1: Plant Cell - Cell wall + Chloroplast must be shown"""
    return """<svg width="320" height="320"><rect x="50" y="50" width="220" height="220" fill="none" stroke="black" stroke-width="3"/><rect x="60" y="60" width="200" height="200" fill="#E8F5E9" stroke="black"/><circle cx="160" cy="160" r="35" fill="#BBDEFB" stroke="black"/><rect x="90" y="90" width="20" height="20" fill="#4CAF50" stroke="black"/><rect x="200" y="120" width="20" height="20" fill="#4CAF50" stroke="black"/><line x1="50" y1="50" x2="20" y2="30" stroke="black"/><text x="5" y="30" font-size="11">Cell Wall</text><line x1="160" y1="125" x2="160" y2="90" stroke="black"/><text x="165" y="90" font-size="11">Nucleus</text><line x1="90" y1="90" x2="60" y2="70" stroke="black"/><text x="5" y="70" font-size="11">Chloroplast</text><line x1="200" y1="120" x2="230" y2="100" stroke="black"/><text x="235" y="100" font-size="11">Chloroplast</text><text x="160" y="300" font-size="14" text-anchor="middle">Plant Cell</text></svg>"""

def s1_bio_leaf():
    """S1: Leaf - External + Vein pattern"""
    return """<svg width="280" height="320"><ellipse cx="140" cy="130" rx="90" ry="100" fill="#A5D6A7" stroke="black" stroke-width="2"/><line x1="140" y1="30" x2="140" y2="230" stroke="#2E7D32" stroke-width="3"/><line x1="140" y1="80" x2="80" y2="100" stroke="#2E7D32"/><line x1="140" y1="80" x2="200" y2="100" stroke="#2E7D32"/><line x1="140" y1="130" x2="70" y2="150" stroke="#2E7D32"/><line x1="140" y1="130" x2="210" y2="150" stroke="#2E7D32"/><line x1="140" y1="230" x2="140" y2="270" stroke="#8D6E63" stroke-width="4"/><text x="145" y="85" font-size="11">Midrib</text><text x="60" y="105" font-size="11">Lateral Vein</text><text x="145" y="270" font-size="11">Petiole</text><text x="140" y="300" font-size="14" text-anchor="middle">Dicot Leaf</text></svg>"""
...
def s2_bio_heart():
    ...
    return """<svg width="320" height="260"><path d="M160 190 Q 70 140 70 100 Q 70 70 100 70 Q 120 70 160 90 Q 200 70 220 70 Q 250 70 250 100 Q 250 140 160 190" fill="#EF9A9A" stroke="black" stroke-width="2"/><line x1="160" y1="90" x2="160" y2="190" stroke="black" stroke-width="2"/><ellipse cx="120" cy="120" rx="25" ry="30" fill="none" stroke="black"/><text x="105" y="125" font-size="10">LV</text><ellipse cx="200" cy="120" rx="25" ry="30" fill="none" stroke="black"/><text x="190" y="125" font-size="10">RV</text><line x1="160" y1="70" x2="160" y2="40" stroke="black" stroke-width="3"/><text x="165" y="35" font-size="11">Aorta</text><line x1="130" y1="70" x2="110" y2="45" stroke="black" stroke-width="2"/><text x="80" y="45" font-size="11">Pulmonary Artery</text><text x="160" y="230" font-size="14" text-anchor="middle">Human Heart</text></svg>"""
...
def s3_bio_photosynthesis(): return """<svg width="360" height="220"><defs><marker id="a" markerWidth="10" markerHeight="10" refX="8" refY="3" orient="auto"><path d="M0,0 L0,6 L9,3 z" fill="black"/></marker></defs><rect x="60" y="90" width="70" height="40" fill="#C8E6C9" stroke="black"/><text x="65" y="115" font-size="10">CO2 + H2O</text><line x1="130" y1="110" x2="180" y2="110" stroke="black" marker-end="url(#a)"/><rect x="180" y="90" width="70" height="40" fill="#FFF9C4" stroke="black"/><text x="185" y="115" font-size="10">C6H12O6 + O2</text><text x="135" y="70" font-size="12">Sunlight + Chlorophyll</text></svg>"""
...
def get_biology_diagram(question):
    ...
    # S1
    if "animal cell" in q: return s1_bio_animal_cell()
    if "plant cell" in q: return s1_bio_plant_cell()
    if "leaf" in q: return s1_bio_leaf()
    if "hand lens" in q or "magnifying" in q: return s1_bio_magnifying()
    if "test tube" in q or "food test" in q: return s1_bio_food_test()
    if "lab coat" in q or "safety" in q: return s1_bio_safety()

    # S2
    if "heart" in q: return s2_bio_heart()
    if "respiratory" in q or "lung" in q or "trachea" in q: return s2_bio_respiratory()
    if "digestive" in q or "stomach" in q: return s2_bio_digestive()
    if "blood cell" in q or "rbc" in q: return s2_bio_blood()
    if "skeleton" in q: return s2_bio_skeleton()
    if "microscope" in q: return s2_bio_microscope()

    # S3
    if "neuron" in q or "nerve cell" in q: return s3_bio_neuron()
    if "kidney" in q: return s3_bio_kidney()
    if "photosynthesis" in q: return s3_bio_photosynthesis()
    if "dna" in q: return s3_bio_dna()
    if "food chain" in q or "ecosystem" in q: return s3_bio_ecosystem()
    if "bacteria" in q: return s3_bio_bacteria()

    # S4
    if "brain" in q: return s4_bio_brain()
    if "eye" in q: return s4_bio_eye()
    if "ear" in q: return s4_bio_ear()
    if "meiosis" in q: return s4_bio_meiosis()
    if "punnett" in q or "genetics" in q: return s4_bio_genetics()
    if "antibody" in q or "immune" in q: return s4_bio_immune()
